...
async function handleGetActivePublicSurvey(req, res) {
  try {
    ...
    // // const apiKey = process.env.JOTFORM_API;
    // // console.log('survey id', activePubSurvey.surveyID);
    ...
    // // console.log('got the following from JotForm.com: ', results.data);
    ...
    //   console.log('sent surveyData to front-end: ', surveyData);
    // } else {
    //   console.log('No results from jot form');
    //   res.status(400).send('No results from Jotform');
    // }
    ...
    //   console.log('inside');
    //   const surveyResponseArr = results.data.content.map(userReponseObj => Object.entries(userReponseObj.answers));
    //   const surveyTrueCountArr = surveyResponseArr.map(answerArr => {
    //     return answerArr.reduce((cntTrue, curVal) => {
    //       return cntTrue + (curVal[1].answer === 'TRUE' ? 1 : 0);
    //     }, 0);
    ...
    //   const surveyData = {
    //     _id: activePubSurvey._id,
    //     surveyID: results.data.content[0].form_id, // 213494408669063 url can be built in front end
    //     createdOn: results.data.content[0].created_at.split(' ')[0], //date survey was created
    //     submissionCount: results.data.resultSet.count, // count of total survey submissions
    //     results: surveyResults, //array of total true counts
    //     ownerID: activePubSurvey.ownerID
    //   };
